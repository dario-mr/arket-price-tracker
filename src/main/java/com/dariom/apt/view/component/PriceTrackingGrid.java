...
public class PriceTrackingGrid extends Grid<PriceTracking> {
    ...
    public PriceTrackingGrid(PriceTrackingService priceTrackingService) {
        ...
        addColumn(new ComponentRenderer<>(tracking -> new Checkbox(tracking.active(), event -> toggleActive(tracking.id(), event.getValue()))))
    ...
    private void loadData() {
        var trackings = new ArrayList<>(priceTrackingService.getAllPriceTrackings()); // mutable list
        dataProvider = new ListDataProvider<>(trackings);
        setDataProvider(dataProvider);
    }

    private void toggleActive(long id, boolean active) {
        priceTrackingService.updateActive(id, active);
    }
